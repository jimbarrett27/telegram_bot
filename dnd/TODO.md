...
- [ ] **#10 Auto-play & AI characters** — 24-hour turn timeout using job_queue. If the active player doesn't act in time, an AI player agent picks a sensible action based on character class, inventory, and situation. Same agent can be used as AI-controlled party members for small groups that need more players.
- [ ] **#11 DM memory persistence** — DM notes tool (write_notes/read_notes backed by DB) for the DM to record key facts (NPC names, plot decisions, item locations). Plus periodic event summarization agent that condenses older events into a "story so far" that stays in the system prompt. Ensures the DM doesn't forget earlier plot points over long adventures.
